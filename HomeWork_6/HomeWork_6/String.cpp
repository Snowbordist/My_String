...
char &String::operator[](const int index) {
	if (index < 0 || index >= size) {
		throw std::exception("Элемента с заданным индексом не существует!");
	}
	else {
		return array[index];
	}
...
BitString &BitString::operator<<(signed int num) {
	if (num < 0) {
		throw std::exception("Количество сдвигов должно быть положительным числом!");
	}
	else {
...
BitString operator | (BitString &one, BitString &two) {
	if (one.getSize() != two.getSize()) {
		throw std::exception("Строки должны быть одного размера!");
	}
	else {
